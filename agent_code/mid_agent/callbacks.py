...
def setup(self):
    ...
    if self.train or not os.path.isfile("my-saved-model.npy"): # how could we train the model again ? 
        ...
        self.model = np.load("my-saved-model_1.npy")
...
def get_state_index(state):
    ...
    for j, a in enumerate(arrangements):
        if tuple(a) in dic.keys():
            # print(f"[!] index: {dic[tuple(a)][value_index]}")
    ...
    *This is not a required function, but an idea to structure your code.*
    ...
    def get_neighbours_values(neighbours):
        ...
        for neighbour in neighbours:
            if neighbour is None:
                values.append(-1)  # coi như tường
            else:
                y, x = neighbour
                value = game_state['field'][y][x]
                if value == 0 and len(game_state['bombs']) > 0:
                    if any([(neighbour == np.array(bomb[0])).all() for bomb in game_state['bombs']]):
                        value = 2  # bomb
                if value == 0 and len(game_state['others']) > 0:
                    if any([(neighbour == np.array(other[3])).all() and (other[3] != game_state['self'][3]) for other in game_state['others']]):
                        value = 1  # opponent
                if value == 0 and game_state['explosion_map'][y][x] == 1:
                    value = 2  # explosion
                values.append(value)
        return values
    ...
    def destroyable_crate(neighbours):
        ...
        if len(crate_coord[0]) > 0:
            ...
            mask = [i for i, neighbour in enumerate(valid_neighbours) if game_state["field"][neighbour[0]][neighbour[1]] != -1]

            if len(mask) > 0:
                temp = [valid_neighbours[i] for i in mask]
                d = np.array([np.min(dist(neighbour, crates)) for neighbour in temp])
                return [mask[np.argmin(d)]]
    ...
    def direction_to_escape_bomb(my_pos, bombs, neighbours, values, j):
        ...
        for idx, neighbour in enumerate(neighbours):
            if (
                ...
                and neighbour.tolist() not in bomb_area.tolist()
                and game_state['field'][neighbour[0]][neighbour[1]] == 0
            ):
                escape_indices.append(idx)

        if len(escape_indices) > 0:
            return j in escape_indices

        # nếu không còn escape rõ ràng, thử đoán
        if (
            neighbours[j] is not None
            and game_state['field'][neighbours[j][0]][neighbours[j][1]] == 0
        ):
            region = get_neighbours(neighbours[j])
            filtered = [r for r in region if r is not None and game_state['field'][r[0]][r[1]] == 0]
            if any((f.tolist() not in bomb_area.tolist()) for f in filtered):
                return True

        # fallback: chọn hướng xa bom nhất
        valid_j = [i for i in range(len(neighbours)) if neighbours[i] is not None and values[i] == 0]
        if not valid_j:
            return False

        temp = [neighbours[i] for i in valid_j]
        d = np.array([dist(bombs[index][0], t) for t in temp])
        max_d = np.max(d)
        max_indices = [valid_j[i] for i in np.where(d == max_d)[0]]

        if len(max_indices) > 1:
            counts = [count_free_tiles(my_pos, neighbours[i]) for i in max_indices]
            return j == max_indices[np.argmax(counts)]
        
        return j in max_indices
    ...
    neighbours = get_fixed_neighbours(my_position)
    ...
    # there are crates/opponents in agent's region, drop bomb
        ...
        neighbour is None or
        danger(neighbour, 0) or
        game_state['explosion_map'][neighbour[0]][neighbour[1]] == 1 or
        game_state['field'][neighbour[0]][neighbour[1]] != 0
        for neighbour in neighbours
    ):

        my_position_value = 2 # should wait

    elif all([value != 0 for value in neighbours_values]):
